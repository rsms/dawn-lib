...
echo "---------- dawn ----------"
DAWN_REV=10ec3cad2d3d44b0e0c82b36859686c11a19ebc7
if _git_dep $DEPS/dawn https://dawn.googlesource.com/dawn.git $DAWN_REV ||
   [ ! -f $DEPS/dawn/configured ]
then
  rm -rf $DEPS/dawn/configured
  if ! (\
  ...
  touch $DEPS/dawn/configured
fi
echo "ready: $DEPS/dawn (git $DAWN_REV)"
